...
<script>
import { BacktestModel } from '../../model/coreindexBacktest'
...
export default {
  ...
  methods: {
    ...
    async getCoreIndexHistoryList() {
      ...
      try {
        const result = await BacktestModel.getBackTestList(params)
        if (result.code == '9999') {
        ...
        this.$message.error('调用回测核心指标查询API异常')
      }
