<template>
  <div class="container">
    ...
    <div class="add-button">
      <el-button type="primary" @click="handAdd" v-permission="'添加期刊内容'">添加内容</el-button>
    </div>
    <div class="table-container">
      <el-table :data="contentList">
        ...
        <el-table-column label="音乐链接" prop="url">
          <template slot-scope="scope">
        ...
        <el-table-column label="点赞数" prop="fav-nums" width="120" />
        ...
        <el-table-column label="操作" fix="right">
          <template slot-scope="scope">
            <el-button @click="handleEdit(scope.row)" v-permission="{ permission: '修改期刊内容', type: 'disabled' }"
              >编辑</el-button
            >
            <el-button
              type="danger"
...
export default {
  ...
  methods: {
    ...
    handAdd() {
      ;(this.dialogTitle = '添加内容'), (this.showDialog = true)
    },
    handleEdit(row) {
    ...
    async confirmAdd() {
      // debugger   断点输入
      // console.log(this.$refs.uploadEle) 控制台日志打印
